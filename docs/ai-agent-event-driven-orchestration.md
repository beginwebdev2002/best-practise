...
> [!IMPORTANT]
> An AI ecosystem MANDATORY resemble a bustling stock exchange floor, not a micromanaged assembly line. Agents must react to state changes, tool executions, and user inputs independently, broadcasting their findings for other specialized agents to consume.
...
> [!IMPORTANT]
> 2. **Idempotent Handlers:** Event handlers MUST be idempotent. Network retries or duplicate LLM outputs MANDATORY not corrupt the system state.
