...
export async function runVibeCheck() {
  ...
  for (const file of modifiedFiles) {
    ...
    techGroups[tech].push({ file, content: await fsPromises.readFile(file, 'utf-8') });
  }

  // Sequential deduplication of syncBenchmarks
  const techMdContents = {};
  for (const tech of Object.keys(techGroups)) {
    const groupFiles = techGroups[tech];
    const combinedMdContent = groupFiles.map(g => g.content).join('\n\n--- \n\n');
    techMdContents[tech] = combinedMdContent;
    await syncBenchmarks(tech, combinedMdContent);
  }

  // Concurrent processing of simulateAIGeneration and analyzeAST
  const analysisResults = await Promise.all(
    modifiedFiles.map(async (file) => {
      ...
      // Local Project for concurrency
      const project = new Project();
      const tempFile = `temp_${tech}_${Date.now()}_${Math.random().toString(36).substring(7)}.ts`;
      const sourceFile = project.createSourceFile(tempFile, generatedCode, { overwrite: true });
      const { total: score, breakdown } = analyzeAST(sourceFile, tech);

      return { file, score, breakdown, generatedCode, tech };
    })
  );
  ...
  for (const result of analysisResults) {
    if (!result) continue;
    const { file, score, breakdown, generatedCode, tech } = result;
