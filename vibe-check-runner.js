...
export async function runVibeCheck() {
  ...
  // Deduplicate by tech and process concurrently for reads
  const techGroups = {};

  const existingFiles = (await Promise.all(modifiedFiles.map(async (file) => {
    return { file, exists: await fileOrDirExists(file) };
  }))).filter(f => f.exists).map(f => f.file);
  ...
  for (const tech of Object.keys(techGroups)) {
    ...
    const files = techGroups[tech];

    // Concurrently read all files and update badges
    const mdContents = await Promise.all(files.map(async (file) => {
    ...
    const generatedCode = await simulateAIGeneration(suiteConfig.golden_prompt, tech, combinedMdContent);
    ...
    if (score >= 95) {
      ...
      try {
        ...
        for (const file of files) {
           if (status.includes(file)) shouldCommit = true;
        }
        if (status.includes('benchmarks/')) shouldCommit = true;

        if (shouldCommit) {
           execFileSync('git', ['commit', '-m', '[chore: benchmark-sync]']);
           execFileSync('git', ['push', 'origin', 'HEAD:main']);
        } else {
           console.log(`Badges already present for ${tech}, skipping commit.`);
        }
      } catch (err) {
