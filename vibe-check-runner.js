...
export async function runVibeCheck() {
  ...
  // Deduplicated syncBenchmarks processed sequentially
  for (const tech of Object.keys(filesByTech)) {
    const techFiles = filesByTech[tech];
    const concatenatedContent = techFiles.map(f => fileContents[f].mdContent).join('\n\n--- \n\n');
    await syncBenchmarks(tech, concatenatedContent);
  }

  // Concurrent execution of I/O and compute tasks via Promise.all
  const analysisResults = await Promise.all(
    Object.keys(fileContents).map(async (file) => {
      ...
      if (!fs.existsSync(suitePath)) {
        ...
        return { file, score: null, breakdown: null, generatedCode: null, tech };
      }
  ...
  // Stateful side effects processed sequentially
