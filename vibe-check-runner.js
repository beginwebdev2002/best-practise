...
export async function runVibeCheck() {
  ...
  // 1. Sequential syncBenchmarks
  for (const tech in filesByTech) {
    console.log(`Syncing benchmarks for ${tech}...`);
    const mdContents = [];
    for (const file of filesByTech[tech]) {
      const content = await fsPromises.readFile(file, 'utf-8');
      mdContents.push(content);
    }
    const combinedContent = mdContents.join('\n\n--- \n\n');
    await syncBenchmarks(tech, combinedContent);
  }

  // 2. Concurrent simulateAIGeneration and analyzeAST
  console.log('Running AI generation and AST analysis concurrently...');
  ...
  for (const tech in filesByTech) {
    ...
    if (fs.existsSync(suitePath)) {
       ...
       console.log(`No benchmark suite found for ${tech}. Skipping AI gen.`);
    }
    ...
    for (const file of filesByTech[tech]) {
      ...
      const p = (async () => {
         ...
         const tempFile = `temp_${tech}_${Math.random().toString(36).substring(7)}.ts`;
  ...
  // 3. Sequential side effects (Git, file updates, GH CLI)
