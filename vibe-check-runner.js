...
export function analyzeAST(sourceFile, tech) {
  ...
  for(const call of callExpressions) {
    if(call.getText().includes('readFileSync') || call.getText().includes('existsSync') || call.getText().includes('writeFileSync')) {
      score.efficiency -= 10;
    }
  }
...
export async function runVibeCheck() {
  ...
  for (const file of modifiedFiles) {
    ...
    filesByTech[tech].push({ file, content, tech });
  }

  for (const tech in filesByTech) {
    const combinedContent = filesByTech[tech].map(item => item.content).join('\n\n--- \n\n');
    await syncBenchmarks(tech, combinedContent);
  }

  const filePromises = [];
  for (const tech in filesByTech) {
    for (const item of filesByTech[tech]) {
      filePromises.push((async () => {
        const { file, content, tech } = item;
        const suitePath = path.join('benchmarks', 'suites', `${tech}.json`);

        if (!await fileOrDirExists(suitePath)) {
          return { file, error: `No benchmark suite found for ${tech}. Skipping.` };
        }

        const suiteConfig = JSON.parse(await fsPromises.readFile(suitePath, 'utf-8'));
        const generatedCode = await simulateAIGeneration(suiteConfig.golden_prompt, tech, content);

        if (!generatedCode) {
          return { file, error: `Failed to generate code for ${tech}.` };
        }

        const localProject = new Project();
        const tempFileName = `temp_${crypto.randomUUID()}.ts`;
  ...
  for (const result of results) {
    ...
    if (score >= 95) {
      ...
      try {
        ...
        try {
            const suitesPath = path.join('benchmarks', 'suites', '*.json');
            const criteriaPath = path.join('benchmarks', 'criteria', '*.json');
            execFileSync('sh', ['-c', `git add ${suitesPath} ${criteriaPath} 2>/dev/null || true`]);
        } catch (e) {}
