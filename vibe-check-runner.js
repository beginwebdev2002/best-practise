...
export async function syncBenchmarks(tech, mdContent, retries = 5, delay = 10000) {
  if (!ai) { console.warn('AI not initialized'); return; }
  try {
...
export function analyzeAST(sourceFile, tech) {
  ...
  for (const param of parameters) {
      if (!param.getTypeNode()) {
          missingTypes++;
          score.type -= 5;
          score.type -= 5; // Enforce explicit parameter types penalty
      }
  }
  if (missingTypes > 0) {
      score.type -= 10;
...
export async function runVibeCheck() {
  ...
  for (const file of modifiedFiles) {
    ...
    if (score >= 95) {
      ...
      const matches = generatedCode.match(/`{3,}/g) || [];
      const maxBackticks = matches.length > 0 ? Math.max(...matches.map(m => m.length)) : 2;
