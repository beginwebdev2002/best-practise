...
const SCORES = { ARCH: 40, TYPE: 30, SECURITY: 20, EFFICIENCY: 10 };
...
export async function syncBenchmarks(tech, mdContent, retries = 5, delay = 10000) {
  try {
    ...
    let parsed;
    try { parsed = JSON.parse(text); } catch (e) { console.warn(`Failed to parse AI response for benchmarks/schema for ${tech}. Using existing if available.`); return; }
    ...
    if (err.status === 429 || err.message?.includes('429') || err.message?.includes('RESOURCE_EXHAUSTED')) {
      ...
      } else { console.error('Max retries reached for syncBenchmarks.'); }
    }
...
export async function simulateAIGeneration(goldenPrompt, tech, mdContent, retries = 5, delay = 10000) {
  try {
    ...
    if (err.status === 429 || err.message?.includes('429') || err.message?.includes('RESOURCE_EXHAUSTED')) {
      ...
      } else { console.error('Max retries reached for AI generation.'); }
    }
...
export function analyzeAST(sourceFile, tech) {
  let score = { arch: SCORES.ARCH, type: SCORES.TYPE, security: SCORES.SECURITY, efficiency: SCORES.EFFICIENCY };
  const decorators = sourceFile.getDescendantsOfKind(SyntaxKind.Decorator);
  const decoratorNames = decorators.map(d => d.getName());

  if (tech === 'nestjs') {
      if (!decoratorNames.includes('Injectable') && !decoratorNames.includes('Controller')) { score.arch -= 10; }
      ...
      for (const classDecl of classDeclarations) {
          ...
          if (classDecorators.some(name => name.startsWith('Is'))) { hasValidation = true; break; }
      }
      if (!hasValidation && decoratorNames.length > 0) { score.arch -= 10; }
  } else if (tech === 'angular') {
      if (!decoratorNames.includes('Component') && !decoratorNames.includes('Injectable')) { score.arch -= 10; }
      if (decoratorNames.includes('Input') || decoratorNames.includes('Output')) { score.arch -= 10; }
  }
  ...
  if (!hasFSD) { score.arch -= 10; }

  const parameters = sourceFile.getDescendantsOfKind(SyntaxKind.Parameter);
  let missingTypes = 0;
  for (const param of parameters) { if (!param.getTypeNode()) { missingTypes++; score.type -= 5; } }
  if (missingTypes > 0) { score.type -= 10; }

  const anyKeywords = sourceFile.getDescendantsOfKind(SyntaxKind.AnyKeyword);
  if (anyKeywords.length > 0) { score.type -= 15 * anyKeywords.length; }

  const explicitParameters = sourceFile.getDescendantsOfKind(SyntaxKind.Parameter);
  for (const param of explicitParameters) { if (!param.getTypeNode()) { score.type -= 5; } }
  ...
  if (awaitExpressions.length > 0 && tryStatements.length === 0) { score.type -= 10; }

  const stringLiterals = sourceFile.getDescendantsOfKind(SyntaxKind.StringLiteral);
  for (const literal of stringLiterals) {
      const text = literal.getText();
      if (text.includes('password') || text.includes('secret') || text.includes('token')) { score.security -= 20; }
  }

  const callExpressions = sourceFile.getDescendantsOfKind(SyntaxKind.CallExpression);
  for(const call of callExpressions) { if(call.getText().includes('readFileSync')) { score.efficiency -= 10; } }

  score.arch = Math.max(0, score.arch); score.type = Math.max(0, score.type); score.security = Math.max(0, score.security); score.efficiency = Math.max(0, score.efficiency);
...
export async function runVibeCheck() {
  ...
  if (modifiedFiles.length === 0) { console.log('No modified .md files found in frontend/ or backend/ within the last 24 hours.'); return; }

  try { execFileSync('git', ['config', '--global', 'user.name', 'github-actions[bot]']); execFileSync('git', ['config', '--global', 'user.email', 'github-actions[bot]@users.noreply.github.com']); } catch (e) { console.warn('Failed to configure git user.'); }
  ...
  for (const file of modifiedFiles) {
    ...
    else { const parts = file.split('/'); if (parts.length > 1) { tech = parts[1]; } else { continue; } }
  ...
  for (const [tech, files] of Object.entries(filesByTech)) {
    let mdContents = [];
    for (const file of files) { mdContents.push(await fsPromises.readFile(file, 'utf-8')); }
    const combinedMdContent = mdContents.join('\n\n--- \n\n');
    ...
    // Concurrent Generation & AST Analysis for files within this tech
    const filePromises = files.map(async (file) => {
      ...
      if (!generatedCode) return { file, success: false, error: 'Failed to generate code' };

      const sourceFile = project.createSourceFile(`temp_${tech}_${Buffer.from(file).toString('base64')}.ts`, generatedCode, { overwrite: true });
    ...
    for (const result of results) {
      if (!result.success) { console.error(result.error); continue; }
      ...
      if (score >= 95) {
        ...
        try {
          ...
          } else { console.log(`Badge already present in ${file}, skipping commit.`); }
        } catch (err) { console.error('Failed to commit or push:', err.message); }
      } else {
        ...
        try { execFileSync('gh', ['issue', 'create', '--title', `Critical Issue: Fidelity Gap for ${file}`, '--label', 'critical,bug', '--body-file', reportPath]); } catch (err) { console.error('Failed to create GitHub Issue:', err.message); }
      }
    }
  }
  if (failedFilesCount > 0) process.exitCode = 1;
}
