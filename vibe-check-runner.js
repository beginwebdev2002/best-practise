...
export async function runVibeCheck() {
  ...
  const filesByTech = new Map();

  // Read all files concurrently and group by tech
  await Promise.all(modifiedFiles.map(async (file) => {
    ...
    if (!filesByTech.has(tech)) {
      filesByTech.set(tech, []);
    }
    filesByTech.get(tech).push({ file, mdContent });
  }));

  // Sync benchmarks sequentially per tech to deduplicate
  const techMdContents = new Map();
  for (const [tech, files] of filesByTech.entries()) {
    const combinedContent = files.map(f => f.mdContent).join('\n\n---\n\n');
    techMdContents.set(tech, combinedContent);
    await syncBenchmarks(tech, combinedContent);
  }

  const fileResults = [];

  // Generate code and analyze AST concurrently
  const processingPromises = [];
  for (const [tech, files] of filesByTech.entries()) {
    const suitePath = path.join('benchmarks', 'suites', `${tech}.json`);

    if (!await fileOrDirExists(suitePath)) {
      console.log(`No benchmark suite found for ${tech}. Skipping generation/analysis.`);
      continue;
    }

    const suiteConfig = JSON.parse(await fsPromises.readFile(suitePath, 'utf-8'));

    for (const { file, mdContent } of files) {
      processingPromises.push((async () => {
        const generatedCode = await simulateAIGeneration(suiteConfig.golden_prompt, tech, mdContent);

        if (!generatedCode) {
          console.error(`Failed to generate code for ${tech} (File: ${file}).`);
          return null;
        }

        const sourceFile = project.createSourceFile(`temp_${tech}_${Buffer.from(file).toString('base64').replace(/=/g, '')}.ts`, generatedCode, { overwrite: true });
        const { total: score, breakdown } = analyzeAST(sourceFile, tech);

        console.log(`Fidelity Score for ${file}: ${score}%`);
        console.log(`Breakdown:`, breakdown);

        return { file, score, breakdown, generatedCode, mdContent };
      })());
    }
  }
  ...
  // Handle side-effects sequentially
  for (const result of results) {
    if (!result) continue;
    const { file, score, breakdown, generatedCode, mdContent } = result;
