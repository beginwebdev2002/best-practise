...
async function runVibeCheck() {
  ...
  // 1. Concurrent and Deduplicated File Loading
  ...
  const techToMdContent = {};
  await Promise.all(
    Object.keys(techToFiles).map(async (tech) => {
      const files = techToFiles[tech];
      const contents = await Promise.all(
        files.map((file) => fsPromises.readFile(file, 'utf-8'))
      );
      // Deduplicate by technology and concatenate markdown contents
      techToMdContent[tech] = contents.join('\n\n--- [Autogenerated separator for context concatenation] ---\n\n');
    })
  );

  // 2. Process sequentially per technology
  for (const tech of Object.keys(techToMdContent)) {
    console.log(`Processing technology: ${tech}...`);
    const mdContent = techToMdContent[tech];
    const filesForTech = techToFiles[tech];
    ...
    console.log(`Fidelity Score for ${tech}: ${score}%`);
    console.log(`Breakdown:`, breakdown);

    if (score >= 95) {
      console.log(`✅ Validation passed for ${tech}. Updating badges and auto-committing.`);

      for (const file of filesForTech) {
        ...
        try { execFileSync('git', ['add', file]); } catch (e) {}
      }
      ...
      try {
        ...
        for (const file of filesForTech) {
            if (status.includes(file)) hasRelevantChanges = true;
        }
        if (status.includes('benchmarks/')) hasRelevantChanges = true;

        if (hasRelevantChanges) {
           execFileSync('git', ['commit', '-m', '[chore: benchmark-sync]']);
           execFileSync('git', ['push', 'origin', 'HEAD:main']);
        } else {
           console.log(`Badges already present and no new benchmarks, skipping commit.`);
        }
      } catch (err) {
      ...
      console.error(`❌ Validation failed for ${tech}. Score below 95%.`);
