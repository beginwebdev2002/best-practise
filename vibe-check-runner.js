...
try {
  ...
  console.warn('Failed to initialize GoogleGenAI. This may cause issues if AI generation is triggered.');
}
...
export async function runVibeCheck() {
  ...
  for (const file of modifiedFiles) {
    ...
    if (score >= 95) {
      ...
      const backtickMatch = generatedCode.match(/`+/g);
      const maxBackticks = backtickMatch ? Math.max(...backtickMatch.map(m => m.length)) : 0;
