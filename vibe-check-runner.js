...
export async function runVibeCheck() {
  ...
  for (const file of modifiedFiles) {
    ...
    // Process syncBenchmarks sequentially for every file
    await syncBenchmarks(tech, mdContent);

    // Immediately read the suite config right after syncBenchmarks
    ...
    fileData.push({ file, tech, mdContent, suiteConfig });
  }

  // Execute I/O and compute tasks concurrently
  const processingResults = await Promise.all(fileData.map(async ({ file, tech, mdContent, suiteConfig }) => {
    if (!suiteConfig) {
