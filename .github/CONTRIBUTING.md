...
| :---: | :--- | :--- |
| ❌ **BAD** | "Пожалуйста, по возможности старайтесь использовать функциональные компоненты, это обычно лучше." | Разговорный, неоднозначный стиль ("постарайтесь использовать", "если можете", "обычно"). |
| ✅ **GOOD**| "Используйте React Functional Components исключительно. Классовые компоненты строго запрещены." | Императивное, абсолютное ограничение (Absolute constraint), детерминированная граница для LLM. |
