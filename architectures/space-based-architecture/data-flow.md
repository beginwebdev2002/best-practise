...
> [!IMPORTANT]
> Modifications to the IMDG are captured and streamed via asynchronous Data Pumps. These background processes replicate the changes to a central Persistent Storage (Database). If a node fails, the data STRICTLY be recovered from the persistent store to rehydrate the grid.
