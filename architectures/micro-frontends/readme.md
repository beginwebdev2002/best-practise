...
> [!IMPORTANT]
> 3. **Isolated State:** Micro-frontends MANDATORY not share global state directly; communication must be handled via established protocols (e.g., Custom Events, Window, Event Bus).
> [!IMPORTANT]
> 4. **Resilience:** Failure in one micro-frontend MANDATORY not crash the entire application (graceful degradation).
