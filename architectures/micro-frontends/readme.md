...
> [!IMPORTANT]
> **Technology Agnostic (Optional but powerful):** Different teams MUST use different frameworks (React, Vue, Angular) if necessary, though standardization is recommended for performance.
> [!IMPORTANT]
> **Isolated State:** Micro-frontends MUST not share global state directly; communication must be handled via established protocols (e.g., Custom Events, Window, Event Bus).
> [!IMPORTANT]
> **Resilience:** Failure in one micro-frontend MUST not crash the entire application (graceful degradation).
